...
int main( int argc, char *argv[] )
{
    ...
    int x, n, i, j, cout, stop, hauteur, largeur, mode;
    char y, c, s, p, nomf[100];
    FILE *f;
    ...
    do {
        affich( &conf, "", 0 );
        printf("a : ajouter pièce   / v : vider case   / g : generer_succ / e : évaluer config\n");
        printf("c : charger fichier / s : sauv fichier / q : quit : ");
        scanf(" %c", &c);
        switch(c) {

            case 'a' : 	printf("Ajout de piece (+/- p y x) : ");
                scanf(" %c %c %c %d", &s, &p, &y, &x);
                if ( s == '+') conf.mat[x-1][y-'a'] = p; else conf.mat[x-1][y-'a'] = -p;
                break;

            case 'v' : 	printf("Vide une case (y x) : ");
                scanf(" %c %d", &y, &x);
                conf.mat[x-1][y-'a'] = 0;
                break;

            case 'g' : 	printf("Générer les successeurs de la configuration courante pour le ");
                printf("Joueur (N ou B) ? : ");
                scanf(" %c", &s);
                if ( s == 'n' || s == 'N' ) mode = MIN; else mode = MAX;
                generer_succ(  &conf, mode, T, &n );
                printf("Affichage des %d successeurs :\n", n );
                while ((c = getchar()) != '\n') ;
                for (i=0; i<n; i++)  {
                    printf("Possibilité %3d\n", i+1 );
                    affich( &T[i], "", 0 );
                    while ((c = getchar()) != '\n') ;
                }
                printf("----------------------\n\n");
            ...
            case 'e' : 	printf("Evaluation de la configuration. Donnez la profondeur et le mode (prof mode) : ");
                scanf(" %d %d", &hauteur, &mode);
                while ((c = getchar()) != '\n') ;
                printf("MinMax avec alpha bêta et profondeur = %d\n", hauteur);
                cout = minmax_ab( &conf, mode, hauteur, -INFINI, +INFINI, largeur );
                printf("coût = %d \n", cout);
                while ((c = getchar()) != '\n') ;
                break;

            case 'c' : 	printf("Charger la conf depuis le fichier : ");
                scanf(" %s", nomf);

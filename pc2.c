...
int estim7(  struct config *conf )
{
    ...
    int i, j, a, b, stop, bns, ScrQte, ScrDisp, ScrDfs, Score =0 , ScoreLib, ScoreCAv, ScrDivers ,divB ,divN;;
    int pionB = 0, pionN = 0, cfB = 0, cfN = 0, tB = 0, tN = 0, nB = 0, nN = 0;
    ...
    if (num_coup < 10 ) {
        Score = (int) (( ScrDisp + 3 * ScrQte + 4 * ScrDfs + ScrDivers ) * 100.0 / ( 32 + 3 * 76 + 8 * 4 + 24));
    }
    else if (num_coup < 30 ){

        Score = (int) ((4 * ScrQte + ScrDisp + ScoreCAv + ScoreLib) * 100.0 / (4 * 76 + 42 + 60 + 56));

    }
    else {

        Score = (int) ((4 * ScrQte + (npmB - npmN) + ScrDivers ) * 100.0 / (4 * 76 + 31 + 24));
